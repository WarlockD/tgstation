...
/obj/machinery/reagentgrinder/ui_interact(mob/user) // The microwave Menu //I am reasonably certain that this is not a microwave
	...
	if(operating || (isAI(user) && machine_stat & MACHINE_STAT_NOPOWER) || !user.canUseTopic(src, !issilicon(user)))
		return
...
/obj/machinery/reagentgrinder/proc/juice()
	power_change()
	if(!beaker || machine_stat & (MACHINE_STAT_NOPOWER|MACHINE_STAT_BROKEN) || beaker.reagents.total_volume >= beaker.reagents.maximum_volume)
		return
...
/obj/machinery/reagentgrinder/proc/mix(mob/user)
	...
	power_change()
	if(!beaker || machine_stat & (MACHINE_STAT_NOPOWER|MACHINE_STAT_BROKEN))
		return
	operate_for(50, juicing = TRUE)
